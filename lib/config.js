...
// 노선 데이터도 설정과 같다. 업데이트는 runtime/에 쓰고, 없으면 저장소 사본을 읽는다.
const RUNTIME_LINE_JSON_PATH = path.join(__dirname, "..", "runtime", "line.json");
const REPO_LINE_JSON_PATH = path.join(__dirname, "..", "line.json");
...
// 설정 페이지(server.js)는 runtime/config.json에 저장한다. 추적 중인 config.json을
// 덮어쓰면 작업 트리가 항상 dirty가 되어 git pull이 막히기 때문이다.
// 페이지에서 한 번도 저장하지 않아 runtime 사본이 없으면 저장소의 config.json을 쓴다.
